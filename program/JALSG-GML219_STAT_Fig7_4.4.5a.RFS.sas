...
* Program name : JALSG-GML219_STAT_Fig7_4.4.5a.RFS.sas
* Author       : AKIKO SAITO
* Date created : 20260504
* Description  : Relapse-free Survival 2年・5年 (FAS + CR到達例) (SAP 4.4.5a)
**********************************************************************;
...
proc printto log="&log.\JALSG-GML219_STAT_Fig7_&DATE..log" new; run;
...
ods rtf file="&output.\JALSG-GML219 Fig7_&DATE..rtf" style=listing;
ods escapechar='^';
footnote2 "^S={just=r} 出力日 &DATE";

/*--- KMカーブ ---*/
title2 'Relapse-free Survival（2年・5年）';
title3 '解析対象集団: FAS 寛解導入療法後 Leukemia-free state 到達例';

ods graphics on / width=18cm height=14cm imagename="Fig7_RFS_FAS_CR";
...
ods select none;
proc lifetest data=FAS_cr alpha=0.05 outsurv=_surv0;
  time RFS_y*rfs_c(0);
run;
ods select all;
...
%macro km_pt(yr);
  data _s&yr.y;
    ...
    where RFS_y <= &yr.;
  run;
  proc sort data=_s&yr.y; by RFS_y; run;
...
proc print data=rfs_est label noobs;
