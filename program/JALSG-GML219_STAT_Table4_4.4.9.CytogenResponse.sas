...
options validvarname=v7 nofmterr nomlogic nosymbolgen nomprint
        ls=120 missing="" pageno=1 nodate nonumber;
...
/*--- WT-1 mRNA を縦持ちに変換 ---*/
data wt1_long;
  length timepoint $40. ord 8. wt1 8.;
  set FAS;

  if wt1_bl      ne . then do; timepoint="Baseline";              ord=1; wt1=wt1_bl;      output; end;
  if wt1_ev1     ne . then do; timepoint="評価1（寛解導入1後）";  ord=2; wt1=wt1_ev1;     output; end;
  if wt1_ev2     ne . then do; timepoint="評価2（寛解導入2後）";  ord=3; wt1=wt1_ev2;     output; end;
  if wt1_ev3     ne . then do; timepoint="評価3（地固め1後）";    ord=4; wt1=wt1_ev3;     output; end;
  if wt1_ev4     ne . then do; timepoint="評価4（地固め2後）";    ord=5; wt1=wt1_ev4;     output; end;
  if wt1_ev5     ne . then do; timepoint="評価5（地固め3後）";    ord=6; wt1=wt1_ev5;     output; end;
  if wt1_relapse ne . then do; timepoint="再発時";                ord=7; wt1=wt1_relapse; output; end;
...
proc tabulate data=wt1_long;
  class ord timepoint / order=data;
  var wt1;
  table
    timepoint='評価時点',
    wt1 * (n*f=8.
...
/*--- WT-1 陽性 (?50 コピー/μg) 例数 ---*/
data wt1_pos;
  set wt1_long;
  length wt1pos $3.;
  if wt1 ne . then wt1pos = ifc(wt1 >= 50, "陽性", "陰性");
  label wt1pos = "WT-1 陽性（?50 コピー/μg）";
run;

title3 'WT-1 陽性率（?50 コピー/μg RNA）';
proc tabulate data=wt1_pos missing;
  class timepoint wt1pos / order=data;
  table timepoint='評価時点',
        wt1pos * (n='件数'*f=8. colpctn='%'*f=8.1)
  / misstext='0';
run;
...
data wt1_cr;
  ...
  if wt1_ev1 ne . then chg_ev1 = wt1_ev1 - wt1_bl;
  if wt1_ev2 ne . then chg_ev2 = wt1_ev2 - wt1_bl;
  if wt1_ev3 ne . then chg_ev3 = wt1_ev3 - wt1_bl;
  if wt1_ev4 ne . then chg_ev4 = wt1_ev4 - wt1_bl;
  if wt1_ev5 ne . then chg_ev5 = wt1_ev5 - wt1_bl;
  label
