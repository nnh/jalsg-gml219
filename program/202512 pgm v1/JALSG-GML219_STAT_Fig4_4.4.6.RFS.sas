...
*
* Program name      : JALSG-GML219_STAT_Fig4_4.4.6.RFS.sas
*
* Author            : AKIKO SAITO
*
...
/*　FAS_LFS */
data FAS_lfs ;  set gml219;
 if FASFL = "Y" and cr1yn="CR";run ;
...
title2 'Relapse free survival' ;
title3 '解析対象集団: FASかつ寛解導入療法でLeukemia-free state到達例' ;

proc lifetest data=fas_lfs plot=survival(atrisk) notable;
 time DFS_y*DFS_c(0) ;
run;
...
/* (結果は表示しない) */
ods select none;
proc lifetest data=fas_lfs alpha=0.05 outsurv=out_s0;
    time DFS_y*dfs_c(0);
run;
ods select all;
...
data out_s1y; set out_s0;
    where DFS_y <= 1.0; /* 1年以下のデータのみを抽出 */
run;
proc sort data=out_s1y; by DFS_y; run;
data Eout_s1y (keep=year survival SDF_LCL SDF_UCL);
    set out_s1y end=final; by DFS_y;
...
data out_s2y; set out_s0;
    where DFS_y <= 2.0; /* 2年以下のデータのみを抽出 */
run;
proc sort data=out_s2y; by DFS_y; run;
data Eout_s2y (keep=year survival SDF_LCL SDF_UCL);
    set out_s2y end=final; by DFS_y;
...
data out_s3y; set out_s0;
    where DFS_y <= 3.0; /* 3年以下のデータのみを抽出 */
run;
proc sort data=out_s3y; by DFS_y; run;
data Eout_s3y (keep=year survival SDF_LCL SDF_UCL);
    set out_s3y end=final; by DFS_y;
