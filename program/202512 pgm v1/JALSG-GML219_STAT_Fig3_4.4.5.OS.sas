...
*
* Program name      : JALSG-GML219_STAT_Fig3_4.4.5.OS.sas
*
* Author            : AKIKO SAITO
*
...
proc printto log="&log.\JALSG-GML219_STAT_Fig3_&DATE.log" new ; run;
...
ods rtf file="&output.\JALSG-GML219 Fig3_&DATE.rtf" style=listing;
ods escapechar='^';
footnote2  "^S={just=r} 出力日 &DATE"  ;

options nodate nonumber;

title2 'Overall survival' ;
title3 '解析対象集団: FAS' ;
...
data out_s1y; set out_s0;
    where OS_y <= 1.0; /* 1年以下のデータのみを抽出 */
run;
proc sort data=out_s1y; by OS_y; run;
data Eout_s1y (keep=year survival SDF_LCL SDF_UCL);
    set out_s1y end=final; by OS_y;
...
data out_s2y; set out_s0;
    where OS_y <= 2.0; /* 2年以下のデータのみを抽出 */
run;
proc sort data=out_s2y; by OS_y; run;
data Eout_s2y (keep=year survival SDF_LCL SDF_UCL);
    set out_s2y end=final; by OS_y;
...
data out_s3y; set out_s0;
    where OS_y <= 3.0; /* 3年以下のデータのみを抽出 */
run;
proc sort data=out_s3y; by OS_y; run;
data Eout_s3y (keep=year survival SDF_LCL SDF_UCL);
    set out_s3y end=final; by OS_y;
