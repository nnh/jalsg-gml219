...
* Program name : JALSG-GML219_STAT_Fig2_4.4.1b.EFS2yr_PPS.sas
* Author       : AKIKO SAITO
* Date created : 20260504
* Description  : 2年EFS (PPS)【参考解析】(SAP 4.4.1b)
**********************************************************************;
...
proc printto log="&log.\JALSG-GML219_STAT_Fig2_&DATE..log" new; run;
...
ods rtf file="&output.\JALSG-GML219 Fig2_&DATE..rtf" style=listing;
ods escapechar='^';
footnote2 "^S={just=r} 出力日 &DATE";

title2 'Event-free Survival（2年・参考）';
title3 '解析対象集団: PPS';

ods graphics on / width=18cm height=14cm imagename="Fig2_EFS2yr_PPS";
proc lifetest data=PPS plots=survival(atrisk cl) notable;
  time EFS_y*efs_c(0);
run;
ods graphics off;

title2 'EFS 時点別推定値（PPS）';
title3 ' ';

ods select none;
proc lifetest data=PPS alpha=0.05 outsurv=_surv0;
  time EFS_y*efs_c(0);
run;
ods select all;
...
proc print data=efs_pps_est label noobs;
